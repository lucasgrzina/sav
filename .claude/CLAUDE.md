...
## Agent Precedence — SAV Domain Agents vs gentle-ai Generic Agents

Not a gentle-ai template block — a project-specific rule. Do not remove on gentle-ai resync; re-add if a future template overwrite drops it.

**Default rule**: SAV domain agents are the primary pipeline for day-to-day work. gentle-ai's generic agents (`sdd-*`, `review-*`, `jd-*`) are additive — they run alongside or after the domain pipeline, they do not replace it, unless explicitly requested below.

| Task type | Primary (default) | gentle-ai generic agent |
| --- | --- | --- |
| New ticket, bug, scoped feature | `ticket-builder` → `arquitecto` → `dev-backend` → `qa-backend`/`qa-frontend` | none by default |
| New backend/frontend module | `backend-module-gen` / `frontend-module-gen` | none |
| Backend/frontend test writing | `backend-tester` / `frontend-tester` | none |
| UI/component work | `ui-specialist` | none |
| Functional spec for complex multi-module feature | `funcional` (functional analysis) then `arquitecto` (technical plan) | `sdd-propose`/`sdd-spec`/`sdd-design`/`sdd-tasks` only if the user explicitly asks for the formal SDD cycle (`/sdd-new`, `/sdd-ff`, or equivalent) |
| Code implementation | `dev-backend` (SAV conventions, `.claude/docs/plans/`) | `sdd-apply` only inside an explicitly chosen SDD track |
| Pre-commit / pre-push review | `qa-backend` / `qa-frontend` as usual | `review-readability` runs additionally per Agent Trigger Rules (cheap, always) |
| Pre-PR on hot paths (auth/update/security/payments) or diffs > 400 lines | `qa-backend` / `qa-frontend` still run | full 4R (`review-risk`, `review-resilience`, `review-readability`, `review-reliability`) runs in addition, per Agent Trigger Rules |
| Post SDD design/apply phase | — (only applies inside an SDD track) | `judgment-day` strongly recommended |

**Conflict rule**: when a request could be routed to both a domain agent and a generic gentle-ai agent, the domain agent wins by default. Route to the gentle-ai generic agent only when:
1. The user explicitly invokes an `/sdd-*` command or asks for "SDD" / "spec-driven development", or
2. The change is substantial and multi-module and the user confirms they want the formal SDD artifact trail (proposal → spec → design → tasks → apply → verify → archive) instead of the informal `funcional`/`arquitecto` plan.

`review-*` and `judgment-day` are the exception — they are always additive quality gates, never a replacement for `qa-backend`/`qa-frontend`, and follow the Agent Trigger Rules cadence (cheap on commit/push, full fan-out only on hot paths/large diffs).

## OpenSpec Removed — Engram Is the Only Artifact Store

Not a gentle-ai template block — a project-specific rule. Do not remove on gentle-ai resync; re-add if a future template overwrite drops it.

This project (shared, multi-collaborator) previously had a standalone OpenSpec CLI integration (`openspec-*` skills, the `opsx:*` plugin from the engram marketplace, and an `openspec/` directory) running alongside gentle-ai's `sdd-*` workflow — two competing spec-driven-dev surfaces on the same conceptual artifacts. It has been removed:

- Deleted: `openspec/` (14 module specs, `config.yaml`, and the pending change `protocol-alerts-enhancements`), `.claude/skills/openspec-apply-change`, `openspec-archive-change`, `openspec-explore`, `openspec-propose`, `openspec-sync-specs`, and `.claude/commands/opsx/`.
- Preserved: all content was migrated into Engram before deletion — the 14 module specs under topic keys `specs/{module}` (e.g. `specs/auth`, `specs/vets`), and the pending change under `sdd/protocol-alerts-enhancements/{proposal,design,tasks,spec}` (standard SDD topic key format, so `/sdd-continue protocol-alerts-enhancements` can resume it directly).

**Rule**: do not offer, ask about, or select `openspec` or `hybrid` as the SDD Artifact Store Mode for this project — it is locked to `engram`. Do not invoke `openspec-*` skills or `opsx:*` plugin commands even if they still appear available in some environment; they no longer have a working `openspec/` directory to operate on in this repo, and the project has standardized on `sdd-*` + Engram.
